...
class SlotODEFunc(eqx.Module):
    """Non-autonomous ODE vector field: ds/dt = f(s, features, t) + g(s, t).
    ...
    All weight matrices (Q, K, V, FF1, FF2) are generated by hypernetworks
    conditioned on t. With Euler integration, each step is:

        slots(t+dt) = slots(t) + dt * [attention(t) + feedforward(t)]
    ...
    norm_attn: eqx.nn.LayerNorm   # pre-attention norm (on slots)
    norm_ff: eqx.nn.LayerNorm     # pre-feedforward norm (on slots)
    scale: float
    ...
    def __call__(self, t: float, slots: jax.Array, args: tuple) -> jax.Array:
        ...
        f_attn = jnp.einsum('bnm,bmd->bnd', att, v)  # [B, N_slots, D]

        # --- g: feedforward ---
        slots_ff = jax.vmap(jax.vmap(self.norm_ff))(slots)          # [B, N_slots, D]
        h = jnp.einsum('bnd,od->bno', slots_ff, W_ff0)              # [B, N_slots, mlp_hidden]
        h = jax.nn.relu(h)
        h = jnp.einsum('bnd,od->bno', h, W_ff1)                     # [B, N_slots, D]
        g_ff = h

        # velocity = attention + feedforward (Euler residual adds to slots)
        return f_attn + g_ff

...
class SlotODEFunc(eqx.Module):
    ...
    def __call__(self, t: float, slots: jax.Array, args: tuple) -> jax.Array:
        ...
        slots_norm = jax.vmap(jax.vmap(self.norm_attn))(slots) # [B, N_slots, slot_dim]
        ...
        f_attn = jnp.einsum('bnm,bmd->bnd', att, v) # [B, N_slots, slot_dim]

        gate_input = jnp.concatenate([slots_norm, f_attn], axis=-1)
        gate = jax.nn.sigmoid(jnp.einsum('bnd,od->bno', gate_input, self.W_gate)) # [B, N_slots, slot_dim]
